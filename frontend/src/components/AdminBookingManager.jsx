...
const AdminBookingManager = () => {
  ...
  const approveBooking = async (booking) => {
    setIsLoading(true);
    try {
      // FIXED: Use PATCH and JSON body to match backend
      const response = await apiRequest(`/bookings/${booking.id}/approve`, 'PATCH', { adminName: 'Admin' });
      if (response.success) {
        ...
        alert('Error: Failed to approve booking: ' + (response.data.message || 'Unknown error'));
      }
    } catch (error) {
      alert('Error: Error approving booking');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const rejectBookingRequest = async () => {
    ...
    try {
      ...
      const response = await apiRequest(
        ...
        'PATCH',
        { adminName: 'Admin', reason: rejectionReason }
      );
      if (response.success) {
        await loadDashboardData();
        ...
        setSelectedBooking(null);
        setRejectionReason('');
        alert('Success: Booking rejected successfully!');
      } else {
  ...
  const cancelApprovedBooking = async (booking) => {
    if (window.confirm(`Warning: Cancel Booking ${booking.bookingReference}?\n\nThis action cannot be undone.`)) {
      setIsLoading(true);
    try {
        // FIXED: Use PATCH and JSON body to match backend
        const response = await apiRequest(`/bookings/${booking.id}/cancel`, 'PATCH', { user: 'Admin' });
        if (response.success) {
          await loadDashboardData();
          ...
          <div className="border-b border-gray-200 bg-gray-50">
            <div className="flex space-x-1 p-4 flex-wrap gap-2">
              <button
                ...
                className={`px-6 py-2.5 rounded-xl font-[800] text-xs uppercase tracking-widest transition-all duration-300 flex items-center space-x-2 ${
                  activeTab === 'pending'
                    ? 'bg-amber-500 text-white shadow-lg shadow-amber-200 border-none'
              ...
              <button
                ...
                className={`px-6 py-2.5 rounded-xl font-[800] text-xs uppercase tracking-widest transition-all duration-300 flex items-center space-x-2 ${
                  activeTab === 'all'
                    ? 'bg-slate-800 text-white shadow-lg shadow-slate-200 border-none'
                    : 'text-slate-500 bg-white hover:bg-slate-50 border border-slate-200'
                }`}
              >
              ...
              <button
                ...
                className={`px-6 py-2.5 rounded-xl font-[800] text-xs uppercase tracking-widest transition-all duration-300 flex items-center space-x-2 ${
                  activeTab === 'REJECTED'
                    ? 'bg-rose-500 text-white shadow-lg shadow-rose-200 border-none'
                    : 'text-slate-500 bg-white hover:bg-slate-50 border border-slate-200'
                }`}
              >
              ...
              <button
                ...
                className={`px-6 py-2.5 rounded-xl font-[800] text-xs uppercase tracking-widest transition-all duration-300 flex items-center space-x-2 ${
                  activeTab === 'RESOURCES'
                    ? 'bg-blue-600 text-white shadow-lg shadow-blue-200 border-none'
                    : 'text-slate-500 bg-white hover:bg-slate-50 border border-slate-200'
                }`}
              >
              ...
              <button
                ...
                className={`px-6 py-2.5 rounded-xl font-[800] text-xs uppercase tracking-widest transition-all duration-300 flex items-center space-x-2 ${
                  activeTab === 'CANCELLED'
                    ? 'bg-slate-500 text-white shadow-lg shadow-slate-200 border-none'
                    : 'text-slate-500 bg-white hover:bg-slate-50 border border-slate-200'
                }`}
              >
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full">
            <div className="p-6">
              ...
              <div className="bg-gray-50 rounded-xl p-4 mb-6">
                ...
                <p className="text-sm text-gray-500">{new Date(selectedBooking.bookingDate).toLocaleDateString()} ΓÇó {selectedBooking.startTime} - {selectedBooking.endTime}</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-4 focus:ring-blue-50 focus:border-blue-400 bg-white text-slate-700 font-bold"
                  >
